...
func UpdatePriceHistoryForTimestamp(appName string, DB *database.Queries, timestamp int64) error {
	...
	err := UpdatePriceHistory(appName, DB, url)
	if err != nil {
		return err
	}
...
func UpdatePriceHistory(appName string, DB *database.Queries, url string) error {
	client := &http.Client{
		Timeout: 15 * time.Second,
	}

	fmt.Printf("Fetching Item Price History Data from %s...\n", url)
